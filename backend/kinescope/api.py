...
class KinescopeClient:
    def __init__(self) -> None:
        ...
        self.session.headers.update(
            {f"Authorization": f"Bearer {settings.kinescope_api_key}"}
        )

    def get_project_video_list(
        self, project_id: str, video_ids: Optional[List[str]] = None
    ) -> List[Video]:
        if video_ids:
...
if __name__ == "__main__":
    video_list = KinescopeClient().get_project_video_list(
        "b0cc85b3-63b1-4a9d-abfc-5c1e02a70daf"
    )
    pprint(video_list)
