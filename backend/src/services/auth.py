...
class AuthService:
    def __init__(self, repo: UserRepository = Depends(), user_service: UserService = Depends()):
        self.repo = repo
    ...
    async def restore_password(self, email: str, user_uuid: uuid.UUID, password: str) -> None:
        ...
        if user.service_uuid != user_uuid:
            raise UnauthorizedException
        await self.user_service.update(user.id, data=AuthUser(hashed_password=pwd_context.hash(password)))
