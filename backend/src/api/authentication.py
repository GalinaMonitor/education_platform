...
async def login_for_access_token(
    user: AuthUser, auth_service: AuthService = Depends(AuthService)
) -> Token:
...
async def create_user(
    user: AuthUser, user_service: UserService = Depends(UserService)
) -> User:
...
async def activate_user(
    email: str, user_uuid: UUID, auth_service: AuthService = Depends(AuthService)
):
    await auth_service.activate_user(email, user_uuid)


@router.post("/users/prepare_restore_password")
async def prepare_restore_password(
    user: AuthUser, auth_service: AuthService = Depends(AuthService)
):
