...
@router.post('/avatar/')
async def update_avatar(photo: UploadFile, current_user: Annotated[User, Depends(get_current_active_user)],
                        session: AsyncSession = Depends(get_session)):
    ...
    await UserService(session).update(id=current_user.id, data=UpdateUser(avatar=f'https://s3.timeweb.com/2d09b0cd-education_platform/{filename}'))
