...
async def change_receive_time(time: Time, id: int, current_user: Annotated[User, Depends(get_current_active_user)],
                              session: AsyncSession = Depends(get_session)):
    time = datetime.strptime(time.time, "%H:%M").time()
    await CourseService(session).change_receive_time(id, user_id=current_user.id,
                                                     receive_time=time)
    return 'Success'
